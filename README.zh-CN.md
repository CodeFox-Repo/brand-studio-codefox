...
Marketing Harness 是一个可安装的 agent skill，用来生产“品牌风格锁定”的宣发图片。安装一次后，在任意业务 repo 里唤起它；agent 会校验品牌 token、准备 campaign、通过本地 image skill/CLI 出图，并把人工验收过的资产发布到当前业务 repo 的资产区。
...
修改官方 brand lock 或发布 live assets 前，用 dry-run render 做人工复核。harness 不假装自动判断图片质量。
